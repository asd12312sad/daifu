    ...
    <el-card class="box-card">
      ...
      <div class="tabswitch" style="display: none">
        <div
          ...
          :key="index"
          :class="
            tabidx == 0 && tabidx == index
              ? 'lefttabactive'
              : tabidx == 1 && tabidx == index
              ? 'righttabactive'
              : ''
          "
          @click="tabidx = index"
      ...
      <div class="tablemain">
        ...
        >
          ...
          <el-table-column label="USDT金额" align="center" prop="usdtBalance" width="120"/>
          <el-table-column
            label="商户订单号"
            align="center"
        ...
        <el-table-column label="手续费" align="center" width="140" prop="feeAmount" />
          ...
          <el-table-column label="回调信息" align="center" prop="returnMsg" width="180">
            <template slot-scope="scope">
              <span>
                {{
                  scope.row.returnMsg && scope.row.returnMsg != "null"
                    ? scope.row.returnMsg
                    : "--"
                }}
              </span>
          ...
          <el-table-column label="审查状态" align="center" prop="status" width="120">
            <template slot-scope="scope">
              <span>
          ...
          <el-table-column label="回调状态" align="center" prop="status" width="120">
            <template slot-scope="scope">
              <span v-if="scope.row.message === 'SUCCESS'">回调成功</span>
              <span v-else-if="!scope.row.message">未回调</span>
              <span v-else>回调失败</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="日志记录" align="center" prop="message" width="120">
            <template slot-scope="scope">
              <span>
                {{
          ...
          <el-table-column label="错误信息" align="center" prop="payResult" width="120"/>
          ...
          <el-table-column label="操作" align="center" prop="buttonFlag" width="120" fixed="right">
            <template slot-scope="scope">
              <div class="caozuopaiban">
                <el-button
                  v-if="
                    scope.row.auditStatus === 1 || scope.row.auditStatus === 2
                  "
                  :disabled="true"
                  size="mini"
                  type="primary"
                  >{{
                    scope.row.auditStatus === 1 ? "已审核" : "已拒绝"
                  }}</el-button
                >
                <el-button
                ...
                <el-button
                  :disabled="
                    scope.row.message !== 'SUCCESS' && scope.row.status === 2
                      ? false
                      : true
                  "
                  size="mini"
              ...
              <el-card class="box-card">
                ...
                <div class="row_between_start">
                  <el-col :span="11">
                    ...
                    <el-row>
                      <span>回调信息:</span>
                      <span>{{ orderdetailitem.returnMsg }}</span>
                    </el-row>
                    <el-row>
                      <span>创建时间:</span>
                    ...
                    <el-row>
                      <span>回调状态:</span>
                      <span v-if="orderdetailitem.message === 'SUCCESS'"
                        >回调成功</span
                      >
                      <span v-else-if="!orderdetailitem.message">未回调</span>
                      <span v-else>回调失败</span>
                    </el-row>
                  </el-col>
                  <el-col :span="11">
...
export default {
  ...
  methods: {
    ...
    getplsh() {
      ...
      this.$apiFun.payOrderaudit(data).then((res) => {
        if (res.code == 200) {
    ...
    huidiaoApi(row) {
      this.$apiFun.getPayOrderHuiDiao({ id: row.payOrderId }).then((result) => {
        if (result.code == 200) {
          this.$message({
            message: "回调成功",
            type: "success",
    ...
    getList() {
      ...
      this.$apiFun.payOrderwholesalelist(this.queryParams).then((result) => {
        if (result.code == 200) {
          ...
          Bus.$emit("daeOrder", this.queryParams.total);
        }
      });
    },
