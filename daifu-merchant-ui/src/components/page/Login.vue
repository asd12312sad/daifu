<template>
  <div class="login-wrapper">
      ...
      <div class="login-form login-child">
        <div class="box-card">
          ...
          >
            ...
            <div class="emailcode" style="display: none">
              <el-form-item prop="code">
                <el-input
                  v-model="ruleForm.googleSign"
                  placeholder="请输入验证码"
                ></el-input>
              </el-form-item>
              <el-form-item>
          ...
          >
            ...
            <div class="emailcode">
              <el-form-item prop="googleSign2">
                <el-input
                  placeholder="请输入谷歌验证码"
                  v-model="ruleForms.googleSign2"
                  @keyup.enter.native="submitForms('ruleForms')"
                ></el-input>
              </el-form-item>
              <el-form-item>
          ...
          <div class="login-btm">
            <div>
              <!-- <router-link to="/forget" class="attach register">
                忘记密码
              </router-link>
              <span>|</span> -->
              <router-link to="/register" class="attach register">
                新用户注册
              </router-link>
            </div>
            <p>©2022 PAY CLOUD</p>
    ...
    <el-dialog
      :title="isGooglecode ? '请使用谷歌验证器扫码绑定' : ''"
    ...
    >
      <div style="display: flex; justify-content: center;flex-direction: column;align-items: center;">
        ...
        <div style="display: flex; justify-content: center;align-items: center;">
          <span style="width:30%;">谷歌验证码:</span>
          <el-input
          placeholder="请输入谷歌验证码"
          type="password"
          v-model="ruleForm.googleSign"
...
export default {
  ...
  methods: {
    ...
    getemailcode() {
      ...
      this.$apiFun.loginCode({ email: this.ruleForm.email }).then((res) => {
        if (res.code == 200) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.$apiFun.login(this.ruleForm).then((res) => {
          if (res.code == 200) {
            localStorage.setItem("tokenType", res.data.tokenType);
            localStorage.setItem("token", res.data.token);
            if (this.checked == true) {
              localStorage.setItem("jizhuemail", this.ruleForm.email);
            }
            this.dialogVisible = false;
            this.ruleForm.googleSign = "";
            this.$router.replace("/");
          } else {
            this.$message.error(res.msg);
          }
  ...
  created() {
    var a = localStorage.getItem("jizhuemail");
    if (a && a != '' && a != undefined) {
      this.checked = true
      this.ruleForm.email = a
    }
    // // 实现页面底部波纹特效
